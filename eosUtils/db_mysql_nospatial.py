...
(
 (min_lat between %s and %s) or
 (max_lat between %s and %s) or
 (min_lat <= %s and max_lat >= %s)
) and
(
 (min_lon between %s and %s) or 
 (max_lon between %s and %s) or
 (min_lon <= %s and max_lon >= %s) or
 (min_lon > max_lon and 
  (
   min_lon between %s and %s or
   (max_lon + 360.) between %s and %s or
   (min_lon <= %s and (max_lon + 360.) >= %s)
  )
 )
);'''
...
def queryDataset(dataset, starttime, endtime, latMin, latMax,
    ...
    conn = MySQLdb.connect(db='test_nospatial', host='127.0.0.1', port=8989,
    ...
    if latMin <= -85. and latMax >= 85. and lonMin <= -175. and lonMax >= 175.:
        ...
        args = (starttime, endtime, starttime, endtime, starttime, endtime,
                latMin, latMax, latMin, latMax, latMin, latMax,
                lonMin, lonMax, lonMin, lonMax, lonMin, lonMax,
                lonMin, lonMax, lonMin, lonMax, lonMin, lonMax)
